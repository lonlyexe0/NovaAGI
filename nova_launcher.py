...
# ═══════════════════════════════════════════════════════════════════════════════
# HUGGINGFACE SÜREKLİ AKIŞ (ENGLISH & TURKISH WIKIPEDIA SUPPORT)
# ═══════════════════════════════════════════════════════════════════════════════
class HuggingFaceAkisi:
    ...
    def __init__(self, hafiza, dur: threading.Event, limit: int = 0, lang: str = "en"):
        self.hafiza  = hafiza
        self.dur     = dur
        self.limit   = limit
        self.lang    = lang
    ...
    def baslat(self):
        ...
        self._thread.start()
        logger.info(f"[HF] ✅ Başladı — {self.dataset_config} ({'English' if self.lang=='en' else 'Türkçe'}) | "
    ...
    def _ic_dongu(self):
        ...
        token = hf_token_al()
        config_adi = self.dataset_config
        logger.info(f"[HF] Bağlanıyor ({config_adi})... (Kimlik: {'Tokenli' if token else 'Anonim'})")
        kwargs = {"split": "train", "streaming": True, "trust_remote_code": True}
        if token:
            kwargs["token"] = token
        ds = load_dataset(self.DATASET_ADI, config_adi, **kwargs)
        logger.info(f"[HF] Bağlandı, {config_adi} akıyor...")
        url_prefix = "hf://en-wiki" if self.lang == "en" else "hf://tr-wiki"
        for veri in ds:
            if self.dur.is_set(): return
            if self.limit > 0 and self.sayac >= self.limit:
                logger.info(f"[HF] Limit ({self.limit:,}) doldu."); return
            metin = veri.get("text","")
            url   = veri.get("url",  f"{url_prefix}/{self.sayac}")
            konu  = veri.get("title","General" if self.lang == "en" else "Genel")
            ...
            try:
                ...
                if self.sayac % self.LOG_ARALIK == 0:
                    s = self.hafiza.istatistik()
                    logger.info(f"[HF] 📚 {self.sayac:>7,} makale ({config_adi}) | "
...
def bilincli_dongu(hafiza, beyin, beden, dur: threading.Event,
    ...
    while not dur.is_set():
        ...
        gecmis = ""
        for a in son[-4:]:
            rol_str = "User" if (eng and a['rol']=='kullanici') else ("Kullanıcı" if a['rol']=='kullanici' else "Nova")
            gecmis += f"{rol_str}: {a['icerik']}\n"

        parcalar = []
        if baglam:
            tag = "[Context:" if eng else "[Bağlam:"
            parcalar.append(f"{tag} {baglam[:250]}]")
        if gecmis:
            parcalar.append(gecmis.strip())
        user_prefix = f"User: {girdi}\nNova:" if eng else f"Kullanıcı: {girdi}\nNova:"
        parcalar.append(user_prefix)
        tohum = "\n".join(parcalar)
        ...
        try:
            ...
            cevap     = _cevap_temizle(cevap_ham, eng=eng)
            print(cevap)
            ...
            # Eylem etiketi
            m = _re.search(r"\[(EYLEM|ACTION):\s*(.+?)\]", cevap, _re.I | _re.DOTALL)
            if m:
                eylem = m.group(2).strip()
                tag_label = "Action" if eng else "Eylem"
                print(f"{R.GRI}  ↳ {tag_label}: {eylem}{R.SIFIR}")
                sonuc = beden.gorevi_coz(eylem)
                res_label = "Result" if eng else "Sonuç"
                print(f"{R.GRI}  ↳ {res_label}: {sonuc[:200]}{R.SIFIR}")
        except Exception as e:
            print(f"{R.KIRMIZI}(Hata: {e}){R.SIFIR}")
        print()


def _cevap_temizle(ham: str, eng: bool = False) -> str:
    import re
    for tag in ["Nova:", "Kullanıcı:", "User:", "[Bağlam:", "[Context:"]:
        i = ham.find(tag)
        if i != -1: ham = ham[:i]
    ham = re.sub(r"\n{3,}", "\n\n", ham).strip()
    ...
    fallback = "I understand. As I learn more, I will give better responses." if eng else "Anlıyorum. Daha fazla öğrendikçe daha iyi yanıtlar vereceğim."
...
{R.SARI}{'━'*62}  NOVA v3 KOMUTLAR  {'━'*62}{R.SIFIR}
  ...
  {R.CYAN}!hf [token]{R.SIFIR}           → Hugging Face token durumu / giriş
  {R.CYAN}!lang [en|tr]{R.SIFIR}         → Dil seçimi / Language switch (English / Türkçe)
...
""")
        ...
        if not arg:
            ...
            yaz(f"🌐 Active language / Aktif dil: {'English (en)' if l=='en' else 'Türkçe (tr)'}", R.CYAN)
        elif arg.lower() in ("en", "eng", "english", "1"):
            set_language("en")
            if hf_akisi: hf_akisi.lang = "en"
            yaz("✓ Switched to English mode. Future Wikipedia streams will use English Wikipedia (20231101.en).", R.YESIL)
        elif arg.lower() in ("tr", "tur", "turkish", "türkçe", "2"):
            set_language("tr")
            if hf_akisi: hf_akisi.lang = "tr"
            yaz("✓ Türkçe moduna geçildi. Wikipedia akışı Türkçe (20231101.tr) olarak ayarlandı.", R.YESIL)
        else:
            yaz("Usage / Kullanım: !lang en  or  !lang tr", R.SARI)

    elif cmd == "hf":
        from hf_auth import hf_durum_metni, hf_token_kaydet_ve_giris, hf_token_sil
        if not arg:
            yaz(hf_durum_metni(), R.CYAN)
        elif arg.lower() in ("sil", "cikis", "çıkış", "logout"):
            hf_token_sil()
            yaz("Hugging Face token'ı silindi. Anonim moda geçildi.", R.SARI)
        else:
...
def _istatistik_yazdir(hafiza, beyin, beden, hf_akisi):
    ...
    lang_str = f"{R.YESIL}English (en) — Wikipedia: 20231101.en{R.SIFIR}" if cur_lang == "en" else f"{R.CYAN}Türkçe (tr) — Wikipedia: 20231101.tr{R.SIFIR}"
    print(f"  {'Dil / Language':<22}: {lang_str}")
    from hf_auth import hf_token_al, _maskeli_token
    token = hf_token_al()
    auth_durum = f"{R.YESIL}🔑 Giriş Yapıldı ({_maskeli_token(token)}){R.SIFIR}" if token else f"{R.GRI}🔓 Anonim Mod{R.SIFIR}"
    print(f"  {'Kimlik':<22}: {auth_durum}")
    if hf:
...
def gui_baslat(hafiza, beyin, beden):
    try:
        ...
        print(f"\n{R.KIRMIZI}❌ GUI açılamadı: {e}{R.SIFIR}")
        if sistem == "Linux":
            print(f"{R.SARI}💡 Linux'ta GUI (Tkinter) kütüphanesi eksik.{R.SIFIR}")
            print(f"   Arch / CachyOS :  sudo pacman -S tk")
            print(f"   Ubuntu / Debian:  sudo apt install python3-tk")
            print(f"   Fedora         :  sudo dnf install python3-tkinter\n")
        elif sistem == "Windows":
            print(f"{R.SARI}💡 Windows'ta Python kurulurken 'tcl/tk and IDLE' seçeneğinin işaretli olduğundan emin olun.{R.SIFIR}\n")
        print(f"{R.CYAN}Terminal modunda devam ediliyor...{R.SIFIR}\n")
        dur = threading.Event()
...
def main():
    ...
    # Bileşenler
    print(f"{R.GRI}[1/4] Hafıza / Memory...{R.SIFIR}")
    from memory import HafizaYoneticisi
    hafiza = HafizaYoneticisi(db_yolu=args.db)

    print(f"{R.GRI}[2/4] Beyin / Brain (Transformer)...{R.SIFIR}")
    from brain import BeynYoneticisi, Config
    Config.device = "cpu"
    beyin = BeynYoneticisi(hafiza)

    print(f"{R.GRI}[3/4] Beden / Body (Hardware+Sensors)...{R.SIFIR}")
    ...
    print(f"{R.GRI}[4/4] HuggingFace akışı hazırlanıyor ({wiki_src})...{R.SIFIR}")
