...
# ═══════════════════════════════════════════════════════════════════════════════
# brain.py  —  Nova Sınırsız Büyüyen Dinamik Sinir Ağı  (Windows / CUDA · DirectML · CPU)
# ═══════════════════════════════════════════════════════════════════════════════
#
...
#   • scaled_dot_product_attention (CUDA/CPU'da Flash / bellek-verimli çekirdekler;
#     DirectML'de uyumlu klasik dikkat yolu kullanılır)
...
# DirectML (AMD / Intel / NVIDIA — Windows) güvenli import
try:
...
_DML_TIPLERI = ("privateuseone", "directml", "privateuseone:0", "dml")
...
def cihaz_coz(ad: str) -> torch.device:
    """Cihaz adını torch.device'a çevirir (DirectML için torch_directml.device())."""
    if str(ad).lower() in _DML_TIPLERI:
        try:
            return torch_directml.device()
        except Exception:
            logger.warning("[Beyin] DirectML başlatılamadı, CPU kullanılacak.")
            return torch.device("cpu")
    return torch.device(ad)


def _dml_mi(t) -> bool:
    return getattr(t, "type", str(t)) == "privateuseone"
...
class DinamikDikkat(nn.Module):
    ...
    def forward(self, x: torch.Tensor,
        ...
        if _dml_mi(x.device):
            # DirectML: SDPA desteği sürüme göre değişir → klasik dikkat
            att = (q @ k.transpose(-2, -1)) * (hd ** -0.5)
            if cache is None:
                maske = torch.ones(T, T, dtype=torch.bool, device=x.device).tril()
                att = att.masked_fill(~maske, float("-inf"))
            att = F.softmax(att, dim=-1)
            if self.training and self._dp > 0:
                att = F.dropout(att, self._dp)
            out = att @ v
        else:
...
class DinamikNovaLM(nn.Module):
    ...
    def uret_stream(self, idx: torch.Tensor, max_new: int = 150, sicaklik: float = 0.65,
        ...
        try:
            ...
            dml = _dml_mi(idx.device)
            gen = idx[:, -ms:]
            gecmis = gen.cpu() if dml else gen          # DirectML: örnekleme CPU'da (uyumlu ve hızlı)
            logits, cache = self._ileri(gen)
            for _ in range(max_new):
                son = logits[:, -1, :]
                nxt = self._ornekle(son.cpu() if dml else son, gecmis, sicaklik, top_k, top_p, rep_ceza)
                gecmis = torch.cat((gecmis, nxt), dim=1)
                nxt = nxt.to(idx.device) if dml else nxt
                gen = torch.cat((gen, nxt), dim=1)
                yield int(nxt[0, 0])
                boy = cache[0][0].shape[2]
                if boy + 1 > ms:
                    # Pencere doldu: son ms token ile önbelleği yeniden kur
                    gen, gecmis = gen[:, -ms:], gecmis[:, -ms:]
                    logits, cache = self._ileri(gen)
                else:
...
# ═══════════════════════════════════════════════════════════════════════════════
# DIRECTML İÇİN SAF GPU ADAMW (CPU'ya geri düşmez)
# ═══════════════════════════════════════════════════════════════════════════════
class DirectMLAdamW(torch.optim.Optimizer):
    """AMD/Intel DirectML için sıfır CPU-fallback AdamW."""
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                denom = (v.sqrt() / ((1.0 - beta2 ** t) ** 0.5)).add_(eps)
...
def _bellek_hatasi_mi(e: BaseException) -> bool:
    ...
    if oom is not None and isinstance(e, oom):
        return True
    return any(k in str(e).lower() for k in ("out of memory", "failed to allocate", "vram", "allocate"))
...
class BeynYoneticisi:
    def __init__(self, hafiza):
        ...
        self.hafiza = hafiza
        self.cfg    = Config()
        self.device = cihaz_coz(self.cfg.device or varsayilan_cihaz())
    ...
    def _optimizer_olustur(self):
        ...
        opt = None
        if _dml_mi(self.device):
            opt = DirectMLAdamW(groups, **kw)
